...
void QueryDriver::writeValue() {
    ...
    if(!theWallet->hasFolder(entryFolder)){
        if (verbose) qDebug() << "Folder does not exist. Creating new folder named" << entryFolder;
    ...
    if(!theWallet->hasEntry(entryName)){
        if (verbose) qDebug() << "Entry" << entryName << "does not exist. Creating default as Password type";
        theWallet->writeEntry(entryName,passwordContents.toUtf8(),Wallet::Password);
